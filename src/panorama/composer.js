...
async function loadLocalFiles() {
    ...
    const panoUrlEntries = [...files].map((file) => {
        const faceName = /(.*?)\.jpe?g/i.exec(file.name)[1];
        return [faceName, URL.createObjectURL(file)];
    });
    const panoUrls = Object.fromEntries(panoUrlEntries);

    const stage = document.querySelector('#stage');
    await stage.addPano(panoUrls);
}
