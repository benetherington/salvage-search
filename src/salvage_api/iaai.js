const IAAI_API = {
    ...
    bonusImageInfo: async (imageInfo) => {
        ...
        try {
            ...
            const spinUrl = new URL(imageInfo.Image360Url);
            const spinPath = /\/([^!]*)/.exec(spinUrl.pathname)[1];
            ...
            // Send request
            const spincarRequest = await fetch(apiUrl, headers);
            return spincarRequest.json();
        } catch (error) {
