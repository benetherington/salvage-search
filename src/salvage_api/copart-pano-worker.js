/*
This code used under GNU Lesser General Public License v3.0
...
/*
This function was adapted to add progress updates.
*/
function renderFace({data: readData, face, rotation, interpolation, maxWidth = Infinity}) {
    ...
    for (let x = 0; x < faceWidth; x++) {
        if (!(x%100)) {
            const percentDone = Math.floor(x/faceWidth*100);
            console.log(percentDone)
            postMessage({percentDone})
        }
    ...
    postMessage({imageData: writeData});
    console.log("All done!")
}

onmessage = function({data}) {
    console.log("data recieved, processing")
    renderFace(data);
};