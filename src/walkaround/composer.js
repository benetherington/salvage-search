...
async function loadLocalFiles() {
    ...
    const files = document.getElementById('pano-files').files;

    // Create object URLs
    const walkaroundUrls = [...files].map((file) => URL.createObjectURL(file));
...
const messageHandler = (message) => {
    ...
    document.getElementById('stage').setAngles(message.walkaroundUrls);
};
...
window.addEventListener('load', () => {
    ...
    // BUTTONS
    document.getElementById('save-view').addEventListener('click', saveThumb);
    document.getElementById('dl-all').addEventListener('click', downloadImages);
    document.getElementById('help').addEventListener('click', toggleHelpPopup);

    // FILES
    document
        .getElementById('pano-files-load')
        .addEventListener('click', loadLocalFiles);
